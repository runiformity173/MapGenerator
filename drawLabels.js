...
function drawStateLabels() {
  ...
  function getLabelPaths() {
    ...
    for (let state = 1;state <= STATE_NUMBER;state++) {
      const offset = getOffsetWidth(state_cells[state]);
      const maxLakeSize = state_cells / 50;
  ...
  function drawLabelPath(letterLength) {
    ...
    for (const [stateId, pathPoints] of labelPaths) {
      ...
      const isInsideState = checkIfInsideState(textElement, angleRad, width / 2, height / 2, f_state, stateId);
  ...
  function checkIfInsideState(textElement, angleRad, halfwidth, halfheight, f_state, stateId) {
    ...
    for (const [x, y] of rotatedPoints) {
      const isInside = f_state[findCell(x, y)] === stateId;
